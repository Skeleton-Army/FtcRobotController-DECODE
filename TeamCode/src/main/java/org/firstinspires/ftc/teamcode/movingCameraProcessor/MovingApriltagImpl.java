...
public class MovingApriltagImpl extends MovingApriltag {
    ...
    public MovingApriltagImpl(Position CameraPos, YawPitchRollAngles cameraOrientation, MatOfDouble distortion, double fx, double fy, double cx, double cy, DistanceUnit outputUnitsLength, AngleUnit outputUnitsAngle, AprilTagLibrary tagLibrary,
                              boolean drawAxes, boolean drawCube, boolean drawOutline, boolean drawTagID, TagFamily tagFamily, int threads, boolean suppressCalibrationWarnings)
    {
        ...
        robotInCameraFrame = OpenGLMatrix.identityMatrix()
                .translated(
                        (float) CameraPos.toUnit(DistanceUnit.INCH).x,
                        (float) CameraPos.toUnit(DistanceUnit.INCH).y,
                        (float) CameraPos.toUnit(DistanceUnit.INCH).z)
    ...
    public void setPoseSolver(AprilTagProcessor.PoseSolver poseSolver) {
        switch (poseSolver) {
            case APRILTAG_BUILTIN:
                setPoseSolver(PoseSolver.APRILTAG_BUILTIN);
                break;
            case OPENCV_ITERATIVE:
                setPoseSolver(PoseSolver.OPENCV_ITERATIVE);
                break;
            case OPENCV_SOLVEPNP_EPNP:
                setPoseSolver(PoseSolver.OPENCV_SOLVEPNP_EPNP);
                break;
            case OPENCV_IPPE:
                setPoseSolver(PoseSolver.OPENCV_IPPE);
                break;
            case OPENCV_IPPE_SQUARE:
                setPoseSolver(PoseSolver.OPENCV_IPPE_SQUARE);
                break;
            case OPENCV_SQPNP:
                setPoseSolver(PoseSolver.OPENCV_SQPNP);
                break;
        }
    }
